...
class Crater:
    ...
    def isErased(self, threshold=0.2):
...
class CraterFactory:
    def CreateCrater(self, range):
        ...
        # radius = cs.uniform_crater_size()
        return Crater(radius, np.random.randint(0, range), np.random.randint(0, range))
...
#     if (craterA.isPointColliding(point, craterB)):
#         circle = plt.Circle((point[0], point[1]), 1, fill=True, color = 'red', alpha=0.5, label='point')
#     else:
